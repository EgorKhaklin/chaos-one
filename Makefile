...
.PHONY: help backend-package unity-app unity-app-bootstrap full-launcher clean-builds

help:
	@echo "Chaos One — root convenience targets"
	@echo ""
	@echo "  make backend-package        Build the chaos-one launcher (PyInstaller)"
	@echo "  make unity-app              Build the Unity .app from an existing scene"
	@echo "  make unity-app-bootstrap    Generate the scene programmatically, then build"
	@echo "  make full-launcher          Backend binary + Unity .app packaged under dist/"
...
unity-app-bootstrap:
	bash unity/build.sh --bootstrap --output unity/dist/ChaosOne.app

full-launcher: backend-package unity-app-bootstrap
